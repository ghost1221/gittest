...
function getDealDate() {
    ...
    getDataOfProfit(dataObject, function (data) {
        showDealHtml(data);
    });
}
...
function openDealInfo(index) {
    ...
    $('.deal-info-head-price').html("+ " + curData['profit']);  // 收益金额
    $('.deal-info-head-status').html(curData['status']) // 交易状态

    var elemAry = $('.deal-info-body-value');
    var valueAry = [
        curData['order_num'], // 流水号
        curData['order_id'],   // 交易流水号
        curData['date'],   // 交易时间
        curData['receive_name'],   // 收款人姓名
        curData['receive_account'],    // 收款账号
        curData['price']  // 交易金额
    ];
...
function showDealHtml(data) {
    ...
    for (var i=0; i<total; i++) {
        html += '<li onclick="openDealInfo('+i+')">\n' +
            ...
            '                                <li class="fontcolor-name">流水号：'+dealList[i]["order_num"]+'</li>\n' +
            '                                <li class="fontcolor-date">'+dealList[i]["date"]+'</li>\n' +
            '                            </ul>\n' +
            '                        </div>\n' +
            '                        <div class="result-li-right">\n' +
            '                            <ul>\n' +
            '                                <li class="fontcolor-price-up">+'+dealList[i]["profit"]+'</li>\n' +
            '                                <li class="fontcolor-status-ing">'+dealList[i]["price"]+'</li>\n' +
            '                            </ul>\n' +
            '                        </div>\n' +
            '                    </li>';
    }
